...
# ─── Geolocation HTTP Server (stdlib only) ───────────────────────────────────
_geo_store = {"lat": None, "lon": None, "status": "idle", "message": ""}
_geo_server_thread = None
_geo_server_instance = None
...
class _GeoHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/":
            html_path = BASE_DIR / "geo.html"
            if html_path.exists():
                self._set_headers(200, "text/html")
                self.wfile.write(html_path.read_bytes())
            else:
                self._set_headers(404)
                self.wfile.write(b"geo.html not found")
        elif self.path == "/get_location":
            self._set_headers()
            self.wfile.write(json.dumps(_geo_store).encode())
        else:
            self._set_headers(404)
    ...
    def do_POST(self):
        if self.path == "/set_location":
            ...
            try:
                ...
                _geo_store["lat"] = data.get("lat")
                _geo_store["lon"] = data.get("lon")
                _geo_store["status"] = data.get("status", "error")
                _geo_store["message"] = data.get("message", "")
            except Exception:
                _geo_store["status"] = "error"
                _geo_store["message"] = "Invalid JSON"
            self._set_headers()
...
def _start_geo_server():
    global _geo_server_thread, _geo_server_instance
    if _geo_server_thread is not None and _geo_server_thread.is_alive():
        return
    _geo_store.update({"lat": None, "lon": None, "status": "idle", "message": ""})
    try:
        _geo_server_instance = socketserver.TCPServer(("127.0.0.1", 8765), _GeoHandler)
    except OSError:
        return  # port already in use, assume another instance is running
    _geo_server_thread = threading.Thread(target=_geo_server_instance.serve_forever, daemon=True)
    _geo_server_thread.start()
...
class DashboardWindow(tk.Tk):
    ...
    def _build_disaster_panel(self, parent):
        ...
        if session["role"] in ("admin", "responder"):
            ...
            self.d_lat_entry = self._ph_entry(ll, "e.g. 19.0760", self.d_vars["lat"],  width=12, side="left", padx=(0,4))
            self.d_lon_entry = self._ph_entry(ll, "e.g. 72.8777", self.d_vars["lon"],  width=12, side="left", padx=(0,4))

            gps_row = tk.Frame(form, bg=SURFACE)
            gps_row.pack(fill="x", pady=(2,0))
            tk.Button(gps_row, text="📍 FETCH GPS LOCATION",
                      font=("Courier", 9, "bold"), bg=ACCENT3, fg=BG,
                      activebackground="#3aa8c7", relief="flat", cursor="hand2",
                      command=self._fetch_gps_for_disaster).pack(side="left", ipady=4, ipadx=4)
            self.d_gps_status = tk.Label(gps_row, text="", font=("Courier", 8),
                                         bg=SURFACE, fg=MUTED)
            self.d_gps_status.pack(side="left", padx=(6,0))
    ...
    def _draw_star(self, canvas, cx, cy, outer_r, inner_r, fill, outline, width=2, rotation=-90):
        """Draw a 5-pointed star polygon on the canvas."""
        import math
        points = []
        for i in range(10):
    ...
    def _redraw_map(self, event=None):
        ...
        for r in self.map_resources:
            ...
            self._draw_star(c, x, y, 10, 5, fill=SURFACE2, outline=SUCCESS, width=2)
    ...
    def _paint_dijkstra_overlay(self, c, w, h, state):
        ...
        for line in lines:
            ...
            if line.get("is_nearest"):
                ...
                self._draw_star(c, rx, ry, 12, 6, fill=SUCCESS, outline="white", width=2)
            else:
                c.create_line(ux, uy, rx, ry, fill=ACCENT, width=2, dash=(4, 4))
                self._draw_star(c, rx, ry, 10, 5, fill=ACCENT, outline="white", width=1)
    ...
    def _fetch_gps_for_disaster(self):
        """Start local HTTP server, open browser for geolocation, and poll for result."""
        _start_geo_server()
        self.d_gps_status.config(text="Waiting for browser...", fg=ACCENT3)
        webbrowser.open("http://127.0.0.1:8765")
        self._geo_poll_start = time.time()
        self._geo_poll_count = 0
        self._poll_geo_location()

    def _poll_geo_location(self):
        """Poll the local server for geolocation result from browser."""
        try:
            with urllib.request.urlopen("http://127.0.0.1:8765/get_location", timeout=2) as resp:
        ...
        if status == "success":
            ...
            if lat is not None and lon is not None:
                # Update entry widgets visually
                self.d_lat_entry.delete(0, "end")
                self.d_lat_entry.insert(0, str(lat))
                self.d_lat_entry.config(fg=TEXT)
                self.d_vars["lat"].set(str(lat))

                self.d_lon_entry.delete(0, "end")
                self.d_lon_entry.insert(0, str(lon))
                self.d_lon_entry.config(fg=TEXT)
                self.d_vars["lon"].set(str(lon))

                self.d_gps_status.config(text="✓ Location auto-filled", fg=SUCCESS)
                return

        if status == "error":
            msg = data.get("message", "Unable to fetch location")
            if "denied" in msg.lower():
                self.d_gps_status.config(text="Location access denied", fg=WARNING)
            else:
                self.d_gps_status.config(text="Unable to fetch location", fg=ACCENT)
            return

        # Timeout after 30 seconds
        if time.time() - getattr(self, "_geo_poll_start", 0) > 30:
            self.d_gps_status.config(text="Unable to fetch location (timeout)", fg=ACCENT)
            return

        self._geo_poll_count = getattr(self, "_geo_poll_count", 0) + 1
        self.after(800, self._poll_geo_location)

    def _locate_me(self):
        """Fetch user's GPS location via browser geolocation using geo.html."""
        global _geo_store
        _geo_store.update({"lat": None, "lon": None, "status": "idle", "message": ""})
        _start_geo_server()
        self.loc_label.config(text="Waiting for browser...", fg=ACCENT3)
        webbrowser.open("http://127.0.0.1:8765")
        self._locate_poll_start = time.time()
        self._poll_locate_me()

    def _poll_locate_me(self):
        """Poll the local server for user geolocation result from browser."""
        try:
            with urllib.request.urlopen("http://127.0.0.1:8765/get_location", timeout=2) as resp:
                data = json.loads(resp.read().decode())
        except Exception:
        ...
        if status == "success":
            ...
            if lat is not None and lon is not None:
                ...
                self.loc_label.config(text=f"📍 {lat:.4f}, {lon:.4f}", fg=ACCENT3)
                ndata = c_nearest(lat, lon)
                if ndata.get("found"):
                    self.n_dist.set(f"{ndata['distance_km']:.1f}")
                    self.n_name.set(
                        f"{TYPE_ICONS.get(ndata['type'],'⚠️')} {ndata['name']}\nSeverity {ndata['severity']}")
                    self.n_gmaps.set(ndata.get("maps_url", ""))
                    self.n_osm.set(ndata.get("osm_url", ""))
                    self._nearest_lat = ndata["lat"]
                    self._nearest_lon = ndata["lon"]
                self._redraw_map()
                return
        ...
        if status == "error":
            msg = data.get("message", "Unable to fetch location")
            if "denied" in msg.lower():
                self.loc_label.config(text="Location access denied", fg=WARNING)
            else:
                self.loc_label.config(text="Unable to fetch location", fg=ACCENT)
            return

        # Timeout after 30 seconds
        if time.time() - getattr(self, "_locate_poll_start", 0) > 30:
            self.loc_label.config(text="Location fetch timeout", fg=ACCENT)
            return

        self.after(800, self._poll_locate_me)
